...
export function getDisciplineDisplayName(expertId) {
...
const SCENE_DEFAULT_EXPERT_IDS = {
...
function buildKnowledgeBase(entry) {
...
function buildMethodology(entry) {
    const specialization = SPECIALIZATION_PROFILES[entry.id];
    switch (entry.toolProfile) {
        case "engineering":
            return [
...
function buildReadOnlyPrompt(_entry) {
    return [
        "工作规则：",
...
function buildEngineeringPrompt(_entry) {
    return [
        "变更与执行规则：",
...
export function buildExpertSystemPrompt(entry) {
...
function scoreToActivationProbability(score) {
    if (score >= 14)
        return 0.92;
    if (score >= 10)
        return 0.82;
...
function formatActivationProbability(probability) {
    return `${Math.round(probability * 100)}%`;
}
function isKnowledgeProductIntent(taskDescription) {
    if (!taskDescription)
        return false;
...
function isSubjectMatterConsultantCandidate(entry) {
    if (entry.systemRole)
        return false;
    if (entry.categoryId === "engineering")
        return false;
    return ![
        "discipline-120",
        "discipline-630",
...
function findPrimarySubjectMatterConsultant(userMessage) {
    if (!isKnowledgeProductIntent(userMessage))
        return undefined;
...
export function evaluateExpertActivation(entry, taskDescription) {
...
function buildActivationGuidance(entry, taskDescription) {
...
function buildConsultingEngagementGuidance(entry, taskDescription) {
    if (!isKnowledgeProductIntent(taskDescription))
        return "";
    ...
    if (!isSubjectMatterConsultantCandidate(entry))
        return "";
    return [
        "【顾问出手要求】",
...
export function buildTaskScopedExpertPrompt(entry, taskDescription) {
    return [
...
export function getExpertSpecializationSummary(id) {
...
export function getSystemExperts() {
    return [...SYSTEM_EXPERTS];
}
export function getDisciplineExperts() {
    return [...DISCIPLINE_EXPERTS];
}
export function getAllExpertEntries() {
    return [...SYSTEM_EXPERTS, ...DISCIPLINE_EXPERTS];
}
export function findExpertEntry(id) {
    return getAllExpertEntries().find((entry) => entry.id === id);
}
export function isImplementationDisciplineExpert(expertId) {
    return /^discipline-(4\d{2}|5\d{2}|610)$/.test(expertId);
}
export function isReviewDisciplineExpert(expertId) {
    return expertId === "discipline-620" || expertId === "discipline-820";
}
export function isDocumentationDisciplineExpert(expertId) {
    return expertId === "discipline-740" || expertId === "discipline-870";
}
export function isCreativeMediaDisciplineExpert(expertId) {
    return expertId === "discipline-760";
}
export function isQuantitativeAnalysisDisciplineExpert(expertId) {
    return expertId === "discipline-910";
}
export function getSceneDefaultExpertIds(scene) {
    return [...(SCENE_DEFAULT_EXPERT_IDS[scene] || [])];
}
function scoreExpert(entry, userMessage) {
...
function inferDispatchIntent(userMessage) {
    const text = userMessage.toLowerCase();
...
function defaultAnchorExperts(userMessage) {
    const text = userMessage.toLowerCase();
...
export function buildDispatchCandidateExperts(userMessage, limit = 6) {
    ...
    for (const item of ranked) {
        if (item.activation.probability < 0.24 || used.has(item.entry.id))
            continue;
...
export function buildExpertToolMap() {
    const result = {
        _common: [],
    };

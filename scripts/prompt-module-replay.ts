...
interface ModuleScenario {
  ...
  expectedIncluded: PromptModuleId[];
  expectedExcluded?: PromptModuleId[];
}
...
interface HistoryScenario {
  ...
  scene: PromptScene;
  task: string;
  expectedModules: PromptModuleId[];
}

interface SessionReplayScenario {
  name: string;
  sessions: unknown;
  expectedTraces: Array<{
    expertId: string;
    scene: PromptScene;
...
const moduleScenarios: ModuleScenario[] = [
  {
...
const sessionReplayScenarios: SessionReplayScenario[] = [
  {
...
for (const scenario of moduleScenarios) {
  const plan = buildExpertPromptPlan(
    scenario.expertId,
    scenario.basePrompt,
    scenario.scene,
    scenario.task,
    scenario.hintModuleIds
  );
  const moduleSet = new Set(plan.moduleIds);
  const extraChars = Math.max(0, plan.prompt.length - scenario.basePrompt.length);
  const baselinePrompt = assemblePromptFromModules(
    scenario.basePrompt,
    getNaiveBaselineModules(scenario.expertId)
  );
...
for (const scenario of intentScenarios) {
  ...
  assert(
    inferred.needsWebSearch === scenario.expected.needsWebSearch
      && inferred.needsCommand === scenario.expected.needsCommand
      && inferred.needsVideoWorkflow === scenario.expected.needsVideoWorkflow,
    `${scenario.name} 的兜底判断与预期不符`
  );
}
...
for (const scenario of historyScenarios) {
  const inferredModules = suggestPromptModuleHintsFromHistory(
    scenario.traces,
    scenario.expertId,
    scenario.scene,
    scenario.task
...
for (const scenario of sessionReplayScenarios) {
  ...
  for (const expectedTrace of scenario.expectedTraces) {
    const matched = traces.find((trace) =>
      trace.expertId === expectedTrace.expertId

...
#[tauri::command]
async fn chat_with_deepseek(messages: Vec<DeepSeekMessage>, api_key: String, model: String) -> Result<String, String> {
...
fn sandbox_edit_file(
    ...
    // 1. 读取现有文件
    let content = sandbox_read_file(project_name.clone(), relative_path.clone(), app_handle.clone())?;

    // 2. 查找 search_text
    if !content.contains(&search_text) {
        return Err(format!(
            "在 {} 中未找到待替换的文本，请检查 search_text 是否与文件中内容完全一致",
            relative_path
        ));
    }

    // 3. 只替换第一次出现（最安全），如果多次出现则报错
    if let Some(pos) = content.find(&search_text) {
        let before = &content[..pos];
        let after = &content[pos + search_text.len()..];
        let new_content = format!("{}{}{}", before, replace_text, after);
        ...
        Err(format!("替换失败：在 {} 中未找到匹配文本", relative_path))
    }
}
...
#[tauri::command]
async fn repo_generate_cards(project_name: String, api_key: String, model: String, app_handle: tauri::AppHandle) -> Result<String, String> {
    let project_dir = get_project_dir(&project_name, &app_handle)?;
    if !project_dir.exists() {
        return Err("项目不存在".to_string());
    }
...
#[tauri::command]
async fn repo_synthesize_wiki(project_name: String, api_key: String, model: String, name: String, app_handle: tauri::AppHandle) -> Result<String, String> {
    let project_dir = get_project_dir(&project_name, &app_handle)?;
    if !project_dir.exists() {
        return Err("项目不存在".to_string());
    }
...
#[tauri::command]
async fn repo_incremental_update(project_name: String, api_key: String, model: String, app_handle: tauri::AppHandle) -> Result<String, String> {
    let project_dir = get_project_dir(&project_name, &app_handle)?;
    if !project_dir.exists() {
        return Err("项目不存在".to_string());
    }
